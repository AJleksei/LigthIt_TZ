...
class Database:
    ...
    def get_user_from_login(self, login):
        ...
        try:
            ...
            for user_db in c.execute('SELECT DISTINCT * FROM Users WHERE login '
                                     'LIKE "' + login + '"'):
                user = User(user_db[0], user_db[1], user_db[2], user_db[3], user_db[4],
                            user_db[5], user_db[6], user_db[7])
        finally:
            conn.close()
        return user
    ...
    def add_user(self, user):
        ...
        try:
            ...
            user_new = c.execute("INSERT INTO Users (id, lastName, firstName, login, password, email, birthDate,"
                                 "mobilePhone) VALUES(NULL, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def edit_user(self, user):
        try:
            ...
            user = c.execute('UPDATE Users SET lastName = ?, firstName = ?, login = ?, password = ?, email = ?, '
                             'birthDate = ?, mobilePhone = ? WHERE id= ? ',
                             (user.lastName, user.firstName, user.login, user.password, user.email, user.birthDate,
                              user.mobilePhone, user.id))
            conn.commit()
        finally:
            conn.close()
        return user
